...
/** Runs `<command> --version` through `probe` and accepts it only when the reported version
 * parses and is at least `MINIMUM_NODE_MAJOR`. Exported so a caller that already knows which
 * binary it wants (`ExtensionRuntime.node()` validating `install.json`'s recorded runtime, P1-5)
 * can apply exactly the same acceptance rule as `resolveNodeRuntime`'s own candidate walk. */
