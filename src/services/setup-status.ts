/**
 * Pure status classification for the status bar and the panel banner, kept free of `vscode` so it
 * can be unit-tested and so `SetupController` (src/services/setup-controller.ts) and the CLI can
 * share it. The distinction matters for the user: a sign-in problem or a browser process failure
 * is operational ("sign in" / "restart"), whereas UI drift is the one case that should send them
 * to the developer. The VS Code extension re-exports this module from `src/extension/status.ts`.
 */
 ...
 * (`SetupController.autoConnect`) resumes only such a workspace -- a repository can commit
