 ...
 * agent URLs and descriptions from its own candidate list by `key`. Host-agnostic (no `vscode`
 * import) so the CLI's terminal renderer can share it; the VS Code extension re-exports this
...
export type PanelState = {
  ...
  discoverySummary?: {
    ...
    descriptions: number;
    partial: boolean;
    /** WP-D: how many store candidates this run could not resolve (see `SetupService.discover()`'s
     * `failedCount`, derived from domain/discovery-warnings.ts's summarizeDiscoveryCompleteness).
     * Metadata only -- a count, never names. Present only alongside `partial: true`, and only when
     * the service itself reported one (an older fallback-derived `partial` may have none). */
    failedCount?: number;
    /** Whether `failedCount` is an exact tally rather than a floor standing in for an unknown
     * number of losses (see `SetupService.discover()`'s `failedCountKnown`). Present only
     * alongside `failedCount`. */
