...
export async function runSelfPrune(
  ...
  if (
    ...
    !(await withYes(deps.prompter, !!options.yes).confirm(
      translator(pickLocaleFromEnv(deps.env))("selfPruneConfirm").replace("{versions}", candidates.join(", "))
    ))
  )
...
export async function runSelfUninstall(
  ...
  if (installJson) {
    ...
    const machineScoped = await removeIntegrations(
      { definition, homeDirectory: deps.homedir(), vscodeUserDirectory, claudeCliAvailable, exec: deps.exec },
    ...
    for (const workspaceRoot of installJson.workspaces) {
      const summary = await removeIntegrations(
        { definition, homeDirectory: deps.homedir(), workspaceRoot },
  ...
  if (options.purgeData) await fs.rm(deps.paths.root, { recursive: true, force: true });
