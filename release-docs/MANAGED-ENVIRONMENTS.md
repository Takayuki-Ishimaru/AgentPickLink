...
は不要）を評価・展開する IT・セキュリティ管理者向けです。v0.2.2 の導入方法を説明します。
...
ポータブルアーカイブにはランタイムと依存関係が含まれ、インストール時の追加ダウンロードは不要です。
Microsoft 365 へのサインイン、エージェントの探索・質問・ファイル取得には、対象サービスへの接続が必要です。
`apl doctor` 自体はリリース確認の通信を行いません。

VS Code 拡張機能は、`agentpicklink.checkForUpdates` が有効な場合、起動時に公開 GitHub Releases API を
確認します。初期値は `true` で、設定から無効にできます。自動インストールは行いません。
